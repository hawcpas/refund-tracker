...
function renderDropoffRequestEmail({
  appName,
  clientName,
  senderName,
  requestUrl,
  ...
  const safeAppName = escapeHtml(appName || "Axume & Associates CPAs");
  ...
  return `
<div style="margin:0;padding:0;background:#f4f6f8;font-family:Arial,Helvetica,sans-serif;color:#101828;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f6f8;padding:28px 12px;">
    <tr>
      <td align="center">
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border:1px solid #e4e7ec;border-radius:10px;overflow:hidden;">
          <tr>
            <td style="padding:24px 28px 18px 28px;border-bottom:1px solid #e4e7ec;">
              <img src="${getEmailLogoUrl()}" alt="${safeAppName}" style="max-height:42px;max-width:260px;" />
            </td>
          </tr>
          <tr>
            <td style="padding:24px 28px;">
              <h1 style="margin:0 0 10px 0;font-size:20px;line-height:1.25;color:#101828;">File request from ${safeSenderName}</h1>
              <p style="margin:0 0 16px 0;font-size:14px;line-height:1.55;color:#475467;">Hello ${safeClientName},</p>
              ${safeMessage ? `<p style="margin:0 0 18px 0;font-size:14px;line-height:1.55;color:#344054;">${escapeHtml(safeMessage).replace(/\n/g, "<br/>")}</p>` : `<p style="margin:0 0 18px 0;font-size:14px;line-height:1.55;color:#344054;">Please use the secure link below to upload the requested files.</p>`}
              <p style="margin:0 0 22px 0;font-size:14px;line-height:1.55;color:#475467;">${safeExpiresLabel}</p>
              <a href="${safeRequestUrl}" style="display:inline-block;background:#0F4FA8;color:#ffffff;text-decoration:none;font-weight:700;font-size:14px;padding:12px 18px;border-radius:8px;">Open file request</a>
              <p style="margin:22px 0 0 0;font-size:12px;line-height:1.5;color:#667085;">This message was generated by ${safeAppName}. Please do not forward this link unless directed by our office.</p>
            </td>
          </tr>
        </table>
      </td>
...
exports.createDropoffRequest = onCall(
  ...
  async (request) => {
    ...
    if (sendEmail && !clientEmail) {
      throw new HttpsError("invalid-argument", "Client email is required to send the request.");
    }
    ...
    await ref.set({
      ...
      fileCount: 0,
      emailed: sendEmail,
      emailedAt: sendEmail ? admin.firestore.FieldValue.serverTimestamp() : null,
    });

    if (sendEmail) {
      let senderName = "Axume & Associates CPAs";
      try {
        const uSnap = await admin.firestore().collection("users").doc(auth.uid).get();
        const u = uSnap.data() || {};
        const first = (u.firstName || "").toString().trim();
        const last = (u.lastName || "").toString().trim();
        const displayName = (u.displayName || "").toString().trim();
        senderName = (first || last) ? `${first} ${last}`.trim() : (displayName || senderName);
      } catch (_) { }

      const expiresLabel = expiresAt
        ? `This file request expires ${expiresAt.toDate().toLocaleString("en-US", {
            month: "short",
            day: "numeric",
            year: "numeric",
            hour: "numeric",
        ...
        : "This file request does not expire automatically.";

      await sendAccountEmail({
        to: clientEmail,
        subject: `${senderName} requested files from you`,
        html: renderDropoffRequestEmail({
          appName: "Axume & Associates CPAs",
